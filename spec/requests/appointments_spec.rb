...
RSpec.describe 'Appointments API', type: :request do
  ...
  let(:a) { { date: Date.today, duration: '1 hr', status: 'booked', user_id: user.id, restaurant_id: restaurant.id } }
  ...
  describe 'POST /appointments' do
    let(:a) { { date: Date.today, duration: '4 hr', status: 'unbooked', user_id: user.id, restaurant_id: restaurant.id } }
    ...
    context 'when the request is invalid' do
      let(:invalid) { { user_id: 1, restaurant_id: 1, date: Date.today, status: 'booked' } }
